document
  ...
  .addEventListener("submit", async function (e) {
    ...
    if (!usernameRegex.test(username)) {
      alert(
        "Username must be 4–20 characters and only use letters, numbers, or underscores."
      );
      return resetButton();
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          window.location.href = "sign_in.php?registered=1";
        }, 1000);
      } else {
...
// Handle terms modal accept button
document
...
// Prevent form submission when clicking on terms link
